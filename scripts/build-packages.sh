...
VERSION=$(awk '/^project\(DSDesk VERSION/{print $3}' "$PROJECT_DIR/CMakeLists.txt")

echo -e "${BOLD}Building DSDesk $VERSION${RESET}"
...
run_pkg_build() {
    ...
    local found
    found=$(find "$OUTPUT_DIR" -maxdepth 1 -name "$glob" 2>/dev/null | wc -l)
    if [ "$found" -eq 0 ]; then
        fail "$label — build succeeded but no package found"
        return 1
    fi

    pass "$label — package written to dist/"
    return 0
}
    ...
    run_pkg_build "Ubuntu DEB" "ubuntu:25.04" \
        "export DEBIAN_FRONTEND=noninteractive
         ...
         apt-get install -y -qq build-essential cmake extra-cmake-modules dpkg-dev \
            qt6-base-dev qt6-declarative-dev qt6-tools-dev-tools file" \
        "cmake -S . -B _build -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=/usr
         cmake --build _build --parallel \$(nproc)
...
build_arch() {
    ...
    log=$("$CONTAINER_RT" run --rm \
        ...
        bash -c "
            ...
            PKGVER=\$(grep -oP 'project\(DSDesk VERSION \K[0-9.]+' /src/CMakeLists.txt)

            # Copy source, stripping any existing build artifacts that would
            # confuse cmake with stale host paths in CMakeCache.txt
            cp -r /src /tmp/DSDesk-\${PKGVER}
            rm -rf /tmp/DSDesk-\${PKGVER}/build \
                   /tmp/DSDesk-\${PKGVER}/_build \
                   /tmp/DSDesk-\${PKGVER}/dist
            tar -czf /tmp/DSDesk-\${PKGVER}.tar.gz -C /tmp DSDesk-\${PKGVER}
            SHA256=\$(sha256sum /tmp/DSDesk-\${PKGVER}.tar.gz | awk '{print \$1}')
            ...
            cp /tmp/DSDesk-\${PKGVER}.tar.gz /home/builder/build/

            # Point PKGBUILD at local tarball with computed SHA
            sed -i \"s|^pkgver=.*|pkgver=\${PKGVER}|\" /home/builder/build/PKGBUILD
            sed -i \"s|^source=.*|source=('DSDesk-\${PKGVER}.tar.gz')|\" /home/builder/build/PKGBUILD
...
build_flatpak() {
    ...
    log=$("$CONTAINER_RT" run --rm --privileged \
        ...
        bash -c "
            ...
            flatpak-builder --force-clean --repo=flatpakrepo build-dir com.dekomote.dsdesk.yml \
                --install-deps-from=flathub --disable-rofiles-fuse
            flatpak build-bundle flatpakrepo dsdesk-${VERSION}.flatpak com.dekomote.dsdesk
            cp dsdesk-*.flatpak /out/
        " 2>&1) && rc=0 || rc=$?
...
# --- AppImage ---
APPIMAGE_BUILDER_IMAGE="dsdesk-appimage-builder:latest"
APPIMAGE_TOOL_CACHE="${XDG_CACHE_HOME:-$HOME/.cache}/DSDesk-appimage-tools"
...
RUN apt-get update -qq && apt-get install -y -qq \
    build-essential cmake extra-cmake-modules wget \
    qt6-base-dev qt6-declarative-dev qt6-tools-dev-tools \
    qt6-wayland libqt6waylandclient6 \
    qml6-module-qtquick qml6-module-qtquick-controls file \
    ...
    log=$("$CONTAINER_RT" run --rm \
        ...
        bash -c "
            ...
            ./linuxdeploy --appdir AppDir \
                --desktop-file AppDir/usr/share/applications/com.dekomote.dsdesk.desktop \
                --icon-file AppDir/usr/share/icons/hicolor/scalable/apps/com.dekomote.dsdesk.svg \
                --plugin qt \
                --output appimage
            cp DSDesk-*.AppImage /out/
        " 2>&1) && rc=0 || rc=$?
...
case "$TARGETS" in
    ...
    appimage-rebuild-cache)
        ...
        ;;
    *)
        echo "Usage: $0 [all|fedora|opensuse|deb|arch|flatpak|appimage|appimage-rebuild-cache]" >&2
        exit 1
        ;;
esac
